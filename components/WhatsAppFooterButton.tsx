...
export function WhatsAppFooterButton({ lang = 'es' }: { lang?: 'es' | 'ca' }) {
  const isCa = lang === 'ca'
  const message = isCa ? WHATSAPP_MESSAGE_CA : WHATSAPP_MESSAGE_ES
  const label = isCa ? 'Escriu-nos per WhatsApp' : 'Escríbenos por WhatsApp'
  ...
  return (
    <a
      ...
      aria-label={label}
      title={label}
      className="whatsapp-footer-button group relative inline-flex items-center gap-3 rounded-full bg-[#25D366] px-5 py-2.5 text-white shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-[0_0_30px_rgba(37,211,102,0.6)] mt-4"
    >
      ...
      <svg
        viewBox="0 0 24 24"
        className="relative z-10 size-4 fill-current flex-shrink-0"
        aria-hidden="true"
      >
      ...
      <span className="relative z-10 text-[11px] font-medium uppercase tracking-[0.18em] whitespace-nowrap">
        {label}
      </span>
    </a>
  )
